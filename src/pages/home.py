...
def ViewHome(page):
    ...
    def create_carousel(page):
        ...
        def update_carousel(index):
            ...
            carousel_image.src = images[index]["src"]
            update_dots()
            page.update()

        def update_dots():
            for i, dot in enumerate(dots.controls):
                dot.bgcolor = "#007BFF" if i == active_index.value else "#CCCCCC"

        # Imagen del carrusel
        carousel_image = ft.Image(
            src=images[0]["src"], width="100%", height=200, fit=ft.ImageFit.COVER
        )
        ...
        dots = ft.Row(
            ...
            controls=[
                ft.Container(
                    ...
                    height=10,
                    border_radius=50,
                    bgcolor="#007BFF" if i == active_index.value else "#CCCCCC",
        ...
        carousel = ft.Column(
            controls=[
                ft.Container(
                    width="100%", height=200, content=carousel_image
                ),
                ft.Container(

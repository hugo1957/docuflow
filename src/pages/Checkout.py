...
def ViewCheckout(page):
            ...
            empty_cart_view = ft.Container(
                ...
                content=ft.Column(
                    controls=[
                        ...
                        ft.Text("Tu carrito está vacío", size=24,
                                weight=ft.FontWeight.BOLD, text_align=ft.TextAlign.CENTER),
                        ft.Text("¡Visita nuestra tienda y agrega productos a tu carrito!",
                                size=16, text_align=ft.TextAlign.CENTER),
                        ft.ElevatedButton(text="Ir a la tienda", on_click=lambda e: page.go(
                            "/"), bgcolor="#e5bc16", color="white", width=150),
                    ],
            ...
            cart_item = ft.Container(
                ...
                content=ft.Column(
                    ...
                    controls=[
                        ...
                        ft.Text(item["name"], size=16, weight=ft.FontWeight.BOLD, max_lines=2,
                                overflow=ft.TextOverflow.ELLIPSIS, text_align=ft.TextAlign.CENTER),
                        ft.Text(f"Precio unitario: ${
                                item['price']}", size=14, text_align=ft.TextAlign.CENTER),
                        ft.Text(f"Cantidad: {
                                item['quantity']}", size=14, text_align=ft.TextAlign.CENTER),
                        ft.Text(f"Total: ${float(
                            item['price']) * item['quantity']:.2f}", size=16, weight=ft.FontWeight.BOLD),
                    ],
                ),
            )
            cart_items_container.controls.append(cart_item)
        total_price = sum(float(item["price"]) *
    ...
    def apply_coupon(e, coupon_code):
        if coupon_code.strip() == "DESCUENTO10":
            page.snack_bar = ft.SnackBar(content=ft.Text(
                "¡Cupón aplicado! 10% de descuento."), bgcolor=ft.colors.GREEN)
            ...
            page.snack_bar = ft.SnackBar(content=ft.Text(
                "Cupón inválido."), bgcolor=ft.colors.RED)
            page.snack_bar.open = True
        page.update()
    ...
    def process_payment(e):
        ...
        if error:
            page.snack_bar = ft.SnackBar(
                content=ft.Text(error), bgcolor=ft.colors.RED)
            page.snack_bar.open = True
            page.update()
            return
        ...
        if order:
            ...
            page.snack_bar = ft.SnackBar(content=ft.Text(
                "Error al registrar la orden."), bgcolor=ft.colors.RED)
            page.snack_bar.open = True
            page.update()
    ...
    def create_order(page):
        cart = page.session.get("cart") or []
        shipping_info = {
            field.label: field.value for field in shipping_info_fields.controls}
    ...
    main_container = ft.Container(
        ...
        content=ft.Column(
            controls=[
                ...
                ft.Column(
                    controls=[
                        ft.Text("Información de Envío", size=20,
                                weight=ft.FontWeight.BOLD),
                        shipping_info_fields,
                    ],
                    spacing=10,
                ),
                ft.Row(controls=[coupon_field,
                       apply_coupon_button], spacing=10),
                ft.ResponsiveRow(alignment=ft.MainAxisAlignment.SPACE_BETWEEN, controls=[
                                 total_price_text, pay_button]),
